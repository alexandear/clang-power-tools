...
Describe "Evaluate-MSBuildCondition" {
    It "When ImportDirectoryBuildProps is true and not exists DirectoryBuildPropsPath returns false" {
        $result = Evaluate-MSBuildCondition "'`$(ImportDirectoryBuildProps)' == 'true' and exists('`$(DirectoryBuildPropsPath)')" 
        
        $result | Should -Be $false
    }

    It "When Miscrosoft SDKs v7.1A installed returns true" {
        $result = Evaluate-MSBuildCondition "'`$(Registry:HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Microsoft SDKs\Windows\v7.1A@InstallationFolder)' != ''"
